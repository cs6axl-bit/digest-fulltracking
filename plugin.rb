...
after_initialize do
  ...
  module ::DigestReport
    ...
    # ===== Link append switch (combined from digest-append-data) =====
    APPEND_LINK_DATA_ENABLED = true

    # When appending data, we add:
    #   isdigest=1
    #   u=<user_id>
    #   dayofweek=<base64url(user_email)>   (kept for backward compatibility)
    #   email_id=<same email_id as tracking pixel / headers>
    #
    ...
    def self.encoded_email_b64url(user_email)
    ...
    def self.rewrite_digest_links!(mail_message, user_id:, user_email:, email_id:)
      return false unless append_link_data_enabled?
      return false if mail_message.nil?
      ...
      dayofweek_val = encoded_email_b64url(user_email)

      html_part =
        if mail_message.respond_to?(:multipart?) && mail_message.multipart?
          mail_message.html_part rescue nil
        else
          # only rewrite if it's HTML
          ct = (mail_message.content_type.to_s rescue "")
          ct.downcase.include?("text/html") ? mail_message : nil
        end

      return false if html_part.nil?

      body = (html_part.body&.decoded.to_s rescue "")
      return false if body.empty?

      # Nokogiri path (preferred)
      if defined?(Nokogiri) && Nokogiri
        begin
          ...
          doc.css("a[href]").each do |a|
            ...
            params << ["u", uid.to_s]    unless params.any? { |k, _| k == "u" }

            if !dayofweek_val.empty?
              params << ["dayofweek", dayofweek_val] unless params.any? { |k, _| k == "dayofweek" }
            end

            # NEW: email_id param (same id as tracking pixel / headers)
            if !email_id.to_s.strip.empty?
              params << ["email_id", email_id.to_s.strip] unless params.any? { |k, _| k == "email_id" }
            end
          ...
          dlog("rewrite_links: OK via Nokogiri uid=#{uid} email_id=#{email_id} len=#{body.length}")
          return true
        rescue StandardError => e
          dlog_error("rewrite_links: Nokogiri error err=#{e.class}: #{e.message}")
      ...
      begin
        new_body = body.gsub(/href="(#{Regexp.escape(base)}[^"]*|\/[^"]*)"/) do
          ...
          # already has any marker? still allow adding missing ones; but avoid doubling email_id etc
          begin
            full = url.start_with?("/") ? (base + url) : url
            uri = URI.parse(full)
          rescue StandardError
            next %{href="#{url}"}
          end
        ...
        html_part.body = new_body
        dlog("rewrite_links: OK via regex uid=#{uid} email_id=#{email_id} len=#{body.length}")
        return true
      rescue StandardError => e
        dlog_error("rewrite_links: regex error err=#{e.class}: #{e.message}")
        return false
      end
    rescue StandardError => e
      dlog_error("rewrite_links: crash err=#{e.class}: #{e.message}")
      false
    end
  end

  # =========================
  # BEFORE send: generate email_id, optionally rewrite links, optionally inject pixel, stamp headers
  # =========================
  DiscourseEvent.on(:before_email_send) do |message, email_type|
    begin
      ...
      # 1) OPTIONAL: append params to internal links (uses SAME email_id)
      appended = false
      if ::DigestReport.append_link_data_enabled? && uid > 0
        appended = ::DigestReport.rewrite_digest_links!(
          message,
          user_id: uid,
      ...
      # 2) OPTIONAL: inject tracking pixel (uses SAME email_id)
      injected = false
      ...
      # 3) Stamp headers (so after_email_send can read reliably)
      ...
      ::DigestReport.dlog(
        "before_email_send: digest to=#{recipient.inspect} user_id=#{uid} " \
        "link_append=#{appended} injected=#{injected} open_used=#{open_used} email_id=#{email_id} " \
